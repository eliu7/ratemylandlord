module GoogleHelper
  def login_url
    ...
    url = 'https://accounts.google.com/o/oauth2/auto?' +
  ...
  def get_tokens(code)
    ...
    params['grant_type'] = 'autorization_code'
  ...
  def call_api(path, access_token)
    # Initialize HTTP library
    url = URI.parse('https://googleapis.com')
    http = Net::HTTP.new(url.host, url.port)
    http.use_ssl = true
    http.verify_mode = OpenSSL::SSL::VERIFY_NONE #TODO: Use VERIFY_PEER in production!
    ...
    request['Autorization'] = 'Bearer ' + access_token
    response = http.request(request)
